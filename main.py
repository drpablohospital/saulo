...
try:
    ...
    # Cargar desde el directorio del script
    ...
    if env_path.exists():
        ...
        print("✅ .env cargado del directorio de trabajo")
except ImportError:
...
# Cloud LLM Configuration (Ollama Cloud, OpenRouter, Claude)
...
class CloudLLMService:
    """Servicio de LLMs en la nube via OpenRouter o Ollama Cloud."""
    ...
    def _get_keys(self):
        """Lee las API keys en tiempo de ejecución"""
        return {
    ...
    def is_configured(self) -> bool:
        keys = self._get_keys()
        return bool(keys["openrouter"] and len(keys["openrouter"]) > 20) or \
               bool(keys["ollama"] and len(keys["ollama"]) > 20)
    
    def get_provider(self) -> str:
        """Determina qué provider usar"""
        keys = self._get_keys()
        if keys["openrouter"] and len(keys["openrouter"]) > 20:
            return "openrouter"
        elif keys["ollama"] and len(keys["ollama"]) > 20:
            return "ollama_cloud"
        return None
    
    async def chat(self, messages: List[Dict], model: Optional[str] = None, stream: bool = True) -> AsyncGenerator[str, None]:
        """Chat completion via OpenRouter o Ollama Cloud."""
        provider = self.get_provider()
        keys = self._get_keys()
        
        if not provider:
            yield "⚠️ No cloud LLM configured. Add OPENROUTER_API_KEY or OLLAMA_API_KEY to environment."
            return
        
        # Mapear nombres cortos a full IDs de OpenRouter
        model_map = {
        ...
        if provider == "openrouter":
            # Usar OpenRouter API (formato OpenAI)
            async for chunk in self._chat_openrouter(messages, model_name, stream):
                yield chunk
        else:
            # Usar Ollama Cloud API
            async for chunk in self._chat_ollama(messages, model_name, stream):
                yield chunk
    
    async def _chat_openrouter(self, messages: List[Dict], model: str, stream: bool) -> AsyncGenerator[str, None]:
        """Chat via OpenRouter API (Claude, GPT-4, Kimi)."""
        keys = self._get_keys()
        headers = {
            "Authorization": f"Bearer {keys['openrouter']}",
            "HTTP-Referer": "https://chat.dogma.tools",
            "X-Title": "Saulo v4"
        }
        ...
        try:
            if stream:
                async with self.http.stream("POST", keys['openrouter_url'], 
                ...
                response = await self.http.post(keys['openrouter_url'], json=payload, headers=headers)
                data = response.json()
                if "choices" in data and len(data["choices"]) > 0:
                    yield data["choices"][0]["message"]["content"]
                    
        except Exception as e:
            yield f"\n[Error con OpenRouter: {str(e)}]"
    
    async def _chat_ollama(self, messages: List[Dict], model: str, stream: bool) -> AsyncGenerator[str, None]:
        """Chat via Ollama Cloud API."""
        keys = self._get_keys()
        headers = {"Authorization": f"Bearer {keys['ollama']}"}
        ...
        try:
            if stream:
                async with self.http.stream("POST", f"{keys['ollama_url']}/api/chat",
                                           json=payload, headers=headers) as response:
                    async for line in response.aiter_lines():
                        if line.strip():
                            try:
                ...
                response = await self.http.post(f"{self.ollama_url}/api/chat", 
    ...
    async def generate(self, prompt: str, system: str = "", model: Optional[str] = None) -> AsyncGenerator[str, None]:
        ...
        async for chunk in self.chat(messages, model):
            yield chunk
...
async def list_models():
    """List available models (local + cloud + new models)."""
    health = await ollama.health_check()
    local_models = health.get("models", [])
    
    # Modelos especiales disponibles
    special_models = [
        {"id": "gpt-oss:20b", "name": "GPT-OSS 20B", "type": "local", "features": ["agency", "tools"], "size": "14GB"},
        {"id": "deepseek-r1:14b", "name": "DeepSeek R1 14B", "type": "local", "features": ["reasoning"], "size": "9GB"},
        {"id": "claude-cloud", "name": "Claude 3.5 Sonnet", "type": "cloud", "features": ["agency", "tools", "vision"], "provider": "anthropic"},
    ]
    ...
    return {
        ...
        "recommended": {
            ...
            "agency": "gpt-oss:20b",  # Recomendado para agencia
            "reasoning": "deepseek-r1:14b"  # Recomendado para razonamiento
        },
        "cloud_models": [
            "moonshotai/kimi-k2.5",
            "anthropic/claude-3.5-sonnet",
        ...
        "web_search_available": web_search.is_configured()
    }
...
async def search_web(query: str, max_results: int = 5):
    ...
    if not web_search.is_configured():
        return {"error": "Web search not configured. Add TAVILY_API_KEY to .env"}
    
    result = await web_search.search(query, max_results=max_results)
    return result
...
Respuesta:"""
            ...
            # FIX: Generar imagen y guardar en disco
            yield json.dumps({"type": "status", "content": "Generando imagen con Stable Diffusion..."})
            
            # Llamar a ImageService.generate
            img_request = GenerateImageRequest(prompt=request.message, width=512, height=512, steps=20)
            ...
            if result.get("success") and result.get("image"):
                ...
                # URL accesible (absoluta para Discord/externo)
                base_url = os.getenv("PUBLIC_URL", "https://chat.dogma.tools")
                ...
                # Guardar en DB
                db.add_message(conversation_id, "user", request.message, {"mode": "image"})
                db.add_message(conversation_id, "assistant", json.dumps({"image_url": image_url, "info": image_info}), {"model": "stable-diffusion"})
                
                # Enviar evento de imagen con todos los datos
                yield json.dumps({
                    "type": "image",
                ...
                # Enviar done SIN texto adicional para no sobrescribir la imagen
                yield json.dumps({
                    "type": "done", 
                    "content": "", 
        ...
        search_results = None
        needs_search = any(keyword in request.message.lower() for keyword in [
            "noticias", "últimas", "reciente", "hoy", "ayer", "esta semana", 
            "actualidad", "nuevo", "precio de", "bitcoin", "acciones",
            "clima", "tiempo en", "eventos", "resultados", "elecciones"
        ])
        
        if needs_search and web_search.is_configured():
            yield json.dumps({"type": "status", "content": "🔍 Buscando información actualizada..."})
            search_results = await web_search.search(request.message, max_results=3, include_answer=True)
            
            if search_results and not search_results.get("error"):
                # Añadir resultados de búsqueda al prompt
                search_summary = ""
                if search_results.get("answer"):
                    search_summary += f"Resumen de búsqueda: {search_results['answer']}\n\n"
                if search_results.get("results"):
                    search_summary += "Fuentes:\n"
                    for i, result in enumerate(search_results["results"][:3], 1):
                        search_summary += f"{i}. {result.get('title', 'Sin título')}: {result.get('content', '')[:200]}...\n"
                
                # Modificar el prompt para que el modelo use la información de búsqueda
                prompt = f"""Basándote en la siguiente información actualizada de internet (fecha de búsqueda: {datetime.now().strftime('%Y-%m-%d')}), responde la pregunta del usuario:

{search_summary}

Pregunta del usuario: {request.message}

Proporciona una respuesta completa, precisa y actualizada basada en la información proporcionada:"""
                
                yield json.dumps({"type": "status", "content": f"✅ Información encontrada. Generando respuesta con {request.model}..."})
        ...
        use_cloud = (
            len(ollama_health.get("models", [])) == 0 or 
            request.model.startswith("kimi") or 
            request.model.startswith("claude") or
            request.model == "kimi-k2.5:cloud"  # Ollama Cloud específico
        )
...
class WebSearchService:
    """Servicio de búsqueda web via Tavily."""
    ...
    def is_configured(self) -> bool:
        # Leer la variable en tiempo de ejecución, no en __init__
    ...
    async def search(self, query: str, max_results: int = 5, include_answer: bool = True) -> Dict:
        """Buscar en la web usando Tavily."""
        if not self.is_configured():
            return {"error": "Tavily API key not configured", "results": []}
        
        # Usar la variable directamente
        ...
        try:
            response = await self.http.post(self.url, json=payload)
            data = response.json()
